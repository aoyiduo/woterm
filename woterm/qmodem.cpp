...
typedef int (*FunZXmitChr)(uchar c, QModem *that);
typedef int	(*FunZXmitStr)(uchar *str, int len, QModem *that);
typedef void (*FunZIFlush)(QModem *that);
typedef void (*FunZOFlush)(QModem *that);
typedef int	(*FunZAttn)(QModem *that);
typedef void (*FunZStatus)(int type, int value, char *status, QModem *that);
typedef void (*FunZFlowControl)(int onoff, QModem *that);
typedef FILE *(*FunZOpenFile)(char *name, ulong crc, QModem *that) ;
typedef int (*FunZWriteFile)(uchar *buffer, int len, FILE *file, QModem *that);
typedef int (*FunZCloseFile)(QModem *that);
typedef void (*FunZIdleStr)(unsigned char *buf, int len, QModem *that);
...
QModem::QModem(bool isTelnet, QObject *parent)
    : QThread(parent)
    , m_bTelnet(isTelnet)
{
...
bool QModem::ZSendFiles(const QStringList &files, Protocol protocol)
{
    if(!m_bInit) {
        return false;
    }
...
bool QModem::ZReceive(const QString &path, Protocol protocol)
{
    if(!m_bInit) {
        return false;
    }
...
bool QModem::onReceive(const QByteArray &buf)
{
    if(!isRunning()) {
        return false;
    }
...
int QModem::ZXmitChr(uchar c)
{
    return 0;
}

int QModem::ZXmitStr(uchar *str, int len)
{
...
void QModem::ZStatus(int type, int value, char *msg)
{
    formatStatus(type, value, msg);
}

void QModem::ZFlowControl(int onoff)
{

}

FILE *QModem::ZOpenFile(char *name, ulong crc)
{
...
int QModem::ZWriteFile(uchar *buffer, int len, FILE *file)
{
    return fwrite(buffer, 1, len, file) == len ? 0 : ZmErrSys;
}

int QModem::ZCloseFile()
{
...
void QModem::ZIdleStr(unsigned char *buf, int len)
{

}

int QModem::_ZXmitChr(uchar c, QModem *that)
{
    return that->ZXmitChr(c);
}

int QModem::_ZXmitStr(uchar *str, int len, QModem *that)
{
    return that->ZXmitStr(str, len);
}

void QModem::_ZIFlush(QModem *that)
{
    that->ZIFlush();
}

void QModem::_ZOFlush(QModem *that)
{
    that->ZOFlush();
}

int QModem::_ZAttn(QModem *that)
{
    return that->ZAttn();
}

void QModem::_ZStatus(int type, int value, char *status, QModem *that)
{
    that->ZStatus(type, value, status);
}

void QModem::_ZFlowControl(int onoff, QModem *that)
{
    that->ZFlowControl(onoff);
}

FILE *QModem::_ZOpenFile(char *name, ulong crc, QModem *that)
{
    return that->ZOpenFile(name, crc);
}

int QModem::_ZWriteFile(uchar *buffer, int len, FILE *file, QModem *that)
{
    return that->ZWriteFile(buffer, len, file);
}

int QModem::_ZCloseFile(QModem *that)
{
    return that->ZCloseFile();
}

void QModem::_ZIdleStr(unsigned char *buf, int len, QModem *that)
{
...
void QModem::status(const QString &msg, bool newLine)
{
    QByteArray buf;
...
void QModem::formatError(int err, const QString &fileName)
{
    switch (err) {
    case ZmDone:
...
void QModem::formatStatus(int type, int value, char *msg)
{
    char tmp[] = "noName";
    char *fileName = tmp;
...
void QModem::onTickerTimeout()
{
    m_ticker.stop();
    m_sigal.wakeAll();
}

void QModem::push(char type, const QByteArray &data)
{
...
bool QModem::pop(char &type, QByteArray &data)
{
    QMutexLocker locker(&m_mutex);
    if(m_queue.isEmpty()) {
        return false;
...
int QModem::XmitFile(const QString &file)
{
    int	done ;
    QFileInfo fi(file);
...
QModem *QWoModemFactory::create(bool isTelnet)
{
    return new QModem(isTelnet, this);
}

void QWoModemFactory::release(QModem *obj)
{
    obj->disconnect();
...
{
    for(QList<QPointer<QModem>>::iterator iter = m_dels.begin(); iter != m_dels.end(); ) {
        QModem *obj = *iter;
        if(obj == nullptr) {
